...
from empire.bot import bot as core
...
def status() -> None:
    ...
    with connect(read_connection_config()) as conn:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT march_id, x_coordinate, y_coordinate, target_level, lord_id, status, duration
                FROM attack
            ...
            for march_id, x, y, level, lid, attack_status, duration in cur.fetchall():
                print(
                    "storm_attack "
                    f"mid={march_id} target={x}:{y} level={level} lid={lid} "
...
def start(args: argparse.Namespace) -> int:
    if should_run_sands(args):
        return sands_proxy.main(["--start", "--max-attacks", str(int(args.max_attacks))])

    args.log_dir.mkdir(parents=True, exist_ok=True)
    core.LOG_FILE = (args.log_dir / datetime.now().strftime("proxy_botv2_%Y%m%d_%H%M%S.log")).open(
        "a",
        encoding="utf-8",
    )
    try:
        ...
        print("\nproxy_botv2 stopped from Ctrl+C", flush=True)
        return 130
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.end:
        ...
        if state.get("mode") == "sands":
            return sands_proxy.main(["--end"])
        core.stop_proxy_transport("manual_end")
        print("proxy_botv2 stopped; mitmproxy session can stay open", flush=True)
        return 0
    return start(args)
