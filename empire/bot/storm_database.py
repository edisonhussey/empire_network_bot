...
def upsert_storm_targets(conn: psycopg.Connection, targets: list[dict[str, Any]]) -> int:
    ...
    with conn.cursor() as cur:
        cur.executemany(
            ...
            ON CONFLICT (kingdom_id, area_type, x_coordinate, y_coordinate) DO UPDATE SET
                object_id = CASE
    ...
    with conn.transaction():
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                WHERE id = %s
                """,
                (epoch + int(reserve_seconds), row[0]),
            )
    return {
        "id": int(row[0]),
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE id = %s
            """,
            (epoch, march_id, attack_name, int(target_id)),
        )
    conn.commit()
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'storm_target', %s, %s, %s, %s, 'sent')
            ON CONFLICT (march_id) DO UPDATE SET
        ...
        cur.execute(
            ...
            WHERE id = %s
            """,
            (int(sent_at), int(march_id), task_name, target_id),
        )
    conn.commit()
    ...
    with conn.cursor() as cur:
        ...
        if target_kind == "storm_target" and target_id is not None:
            cur.execute(
                ...
                WHERE id = %s
                """,
                (result_flag, epoch, Jsonb(raw_result), int(target_id)),
            )
    conn.commit()
    return True
